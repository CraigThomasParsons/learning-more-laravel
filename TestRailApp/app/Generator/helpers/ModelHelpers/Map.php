...
use Generator\model\MongoDatabase\TileType;
use Generator\model\MongoDatabase\TileRepository;
use Generator\model\MongoDatabase\CellRepository;

use Generator\model\MongoDatabase\Tile as TileRecord;
use Generator\model\MongoDatabase\Cell as CellRecord;
 ...
 *
 * Note to self: The trick to getting the arrayaccess to work the way I want it to.
...
{
    /**
     * Represents the Map record of table map Generator\model\MongoDatabase\Map
     *
     * @var $databaseRecord
     */
    ...
    public function setTilePropertiesOfCell(
        ...
        foreach ($arrNewValues as $strProperty => $mxdValue) {
            ...
            $this->arrCells[$intXaxisCoordinate][$intYaxisCoordinate]
    ...
    public function setTilePropertiesOfMapAndSave($tile, $intXaxisCoordinate, $intYaxisCoordinate, $arrNewValues)
    {
        ...
        foreach ($arrNewValues as $strProperty => $mxdValue) {
            $strSetter = 'set'.$strProperty;
            $this->arrCells[$intCellXaxisCoordinate][$intCellYaxisCoordinate]
                 ->arrTmpTiles[$intXaxisOffsetCoord][$intYaxisOffsetCoord]
                 ->$strSetter($mxdValue);
        }
        ...
        foreach ($this->arrTiles as $intXaxisCoordinate => $arrColumn) {
            foreach ($arrColumn as $intYaxisCoordinate => $tile) {
                ...
                if (!$tileRecord->id) {
                    ...
                    // Needed to inval it, because it wasn't being stored in Mongto as an integer.
                    // The value could possibly be coming in from a html form.
                    ...
                    if ($cellRecord->id) {
                        ...
                        // Needed to inval it, because it wasn't being stored in Mongto as an integer.
                        $cellRecord->mapId = intval($tileRecord->mapId);
                        $cellRecord->coordinateX = $intCellXaxisCoordinate;
                        $cellRecord->coordinateY = $intCellYaxisCoordinate;
    ...
    public function setTilePropertiesOfMap(Tile $tile, $intXaxisCoordinate, $intYaxisCoordinate, $arrNewValues)
    {
        ...
        return $this->arrCells[$intCellXaxisCoordinate][$intCellYaxisCoordinate]
                    ->arrTmpTiles[$intXaxisOffsetCoord][$intYaxisOffsetCoord];
    }
